...
export default function AnimatedRoundedLoader() {
  ...
  const getDuration = (index: number): number => {
    ...
    // Fast start (build trust)
    if (step >= 1 && step <= 3) return 2000; // 2s

    // Heavy AI thinking
    if (step >= 4 && step <= 8) return 6500; // 6.5s

    // Wrapping up
    if (step >= 9 && step <= 11) return 3000; // 3s

    // Final confirmation
    if (step === 12) return 1800; // quick finish
  ...
  return (
    <div className="flex-1 w-full flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      <style>{`
        @keyframes fadeIn {
          from {
            opacity: 0;
