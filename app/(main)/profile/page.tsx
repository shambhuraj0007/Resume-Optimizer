    ...
    <div className="container min-h-screen mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="space-y-6">
          ...
          <Card className="h-fit overflow-visible">
            <CardHeader className="text-center pb-4 overflow-visible">
              ...
              {/* FIXED SECTION: Used 'span' instead of 'div' to prevent HTML nesting errors */}
              <CardDescription className="space-y-2 flex flex-col items-center">
                <span className="flex items-center justify-center gap-2">
                  <User className="w-4 h-4 flex-shrink-0" />
                  <span className="text-sm text-foreground">@{session.user?.name ?? 'username'}</span>
                </span>
                <span className="flex items-center justify-center gap-2">
                  <Mail className="w-4 h-4 flex-shrink-0" />
                  <span className="text-xs sm:text-sm break-all px-2 text-foreground">
            ...
            <CardContent className="space-y-4">
              <div>
                <div className="flex items-baseline gap-2 flex-wrap">
                  <span className="text-4xl font-bold text-foreground">{balance?.credits ?? 0}</span>
                  <span className="text-muted-foreground text-sm font-medium">
                    {balance?.credits === 1 ? 'Scan' : 'Scans'} remaining
                  </span>
                </div>
                ...
                {balance?.nextCreditReset && (
                  <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                    <Calendar className="h-4 w-4 flex-shrink-0" />
                    <span>Free Scans renew on {format(new Date(balance.nextCreditReset), 'MMM dd, yyyy')}</span>
                  </div>
                )}

                {balance?.hasExpired && (
                  <p className="text-sm text-red-500 mt-3">
                    Your Scans have expired
                  </p>
                )}
              </div>
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                          <div className="flex items-start gap-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              <h3 className="font-semibold text-base sm:text-lg break-all text-card-foreground">
                                {resume.id}
                              </h3>
      ...
      <UpgradeModal
        ...
        onSuccess={() => {
          ...
          toast({
            title: "Scans Added!",
            description: "Your scans have been successfully added to your account.",
          });
        }}
      />
