...
export default function AuthCard() {
  ...
  const [mode, setMode] = useState<"signin" | "signup">(
    searchParams.get("mode") === "signup" || searchParams.has("signup") ? "signup" : "signin"
  );
      ...
      if (res?.error) {
        ...
        if (typeof window !== 'undefined' && (window as any).dataLayer) {
          try {
            ...
            console.error("Failed to fetch session for login event", e);
  ...
  const onSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        setMsg({ success: data.message || "Account created! Please check your email to verify your account, then sign in." });

        // ------------------------------------------------------------------
        // GTM EVENT: signup_done (email)
        // ------------------------------------------------------------------
        if (typeof window !== 'undefined' && (window as any).dataLayer) {
          (window as any).dataLayer.push({
            event: 'signup_done',
            ...
            method: 'email',
            source: searchParams.get("callbackUrl")?.includes("pricing") ? "pricing" : "direct"
          });
        }
  ...
  // Helper to switch modes efficiently
  ...
  const subtitle = mode === "signup" ? "Free Scans every month" : "Sign in to continue";
  ...
  return (
    <div className="min-h-screen flex flex-col items-center pt-2 md:pt-4 bg-gradient-to-b from-white to-gray-50 dark:from-gray-950 dark:to-gray-900 p-4">
      <motion.div
      ...
      >
        ...
        <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-md border border-gray-200 dark:border-gray-800 shadow-xl rounded-2xl p-5 space-y-3">
          <div className="bg-gray-100 dark:bg-gray-800/50 p-1.5 rounded-xl grid grid-cols-2 relative">
            <button
              onClick={() => handleModeSwitch("signup")}
              type="button"
              className="relative z-10 py-2.5 text-sm font-semibold transition-colors duration-200 text-center"
            >
              <span className={mode === "signup" ? "text-gray-950 dark:text-white" : "text-gray-500 dark:text-gray-400"}>
                Sign Up
              </span>
              {mode === "signup" && (
                <motion.div
            ...
            <button
              ...
              className="relative z-10 py-2.5 text-sm font-semibold transition-colors duration-200 text-center"
            >
              <span className={mode === "signin" ? "text-gray-950 dark:text-white" : "text-gray-500 dark:text-gray-400"}>
                Sign In
              </span>
              {mode === "signin" && (
                <motion.div
          ...
          >
            ...
            {/* Email field - Added 'layout' so it slides when Name appears */}
            ...
            {/* Password field - Added 'layout' */}
            {/* Password field - Added 'layout' */}
            <motion.div layout className="grid gap-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="password">{mode === "signin" ? (<p>Password</p>) : (<p>Password (use 8+ characters)</p>)}</Label>
                {mode === "signin" && (
                  <Link
              ...
              {/* Helper text vs Error message logic */}
            ...
            {/* Submit button - Added 'layout' so it slides down when Confirm appears */}
            <motion.div layout whileHover={!isLoading ? { y: -1 } : {}} whileTap={!isLoading ? { scale: 0.98 } : {}}>
