 ...
 * It takes the callback (GET or POST) and redirects the user to
 * /payment/success using a standard GET redirect (303 for POST).
 */
export async function POST(req: NextRequest) {
    try {
        ...
        const redirectUrl = new URL(`/payment/success?${searchParams}`, baseUrl);
        ...
        return NextResponse.redirect(new URL("/payment/success", req.url).toString());
    }
}
