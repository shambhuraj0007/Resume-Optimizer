...
export async function POST(req: NextRequest) {
  try {
    ...
    if (event.event === 'payment.captured') {
      ...
      // Update user region and paid status (for first-time payment locking)
      const user = await User.findById(transaction.userId);
      if (user && !user.region) {
        // Lock region based on currency (Razorpay/Cashfree is India-only)
        user.region = 'IN'; // Razorpay operates in India with INR
        user.isPaidUser = true;
        await user.save();
        console.log(`Locked user ${user.email} to region: IN`);
      }
