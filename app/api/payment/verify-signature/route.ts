...
export async function GET(req: NextRequest) {
    try {
        ...
        // Resolve plan details for response
        ...
        if (cfSubscriptionId && subscriptionStatusUpdated) {
            // Try to find the transaction to get exact details
            const tx = await Transaction.findOne({ paypalSubscriptionId: cfSubscriptionId });
            if (tx) {
                resolvedAmount = tx.amount;
                resolvedCurrency = tx.currency;
                resolvedCredits = tx.credits;
                resolvedPlanId = tx.packageType;
            }
        }
