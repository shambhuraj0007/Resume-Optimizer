...
export async function POST(req: NextRequest) {
  try {
    ...
    const deterministicSuggestions = generateDeterministicSuggestions(
      ...
      llmData.titles.jdTitle || "Target Role"
    );

    // Merge suggestions: Deterministic first (Tickable), then LLM (Text/Other)
    // Filter out malformed suggestions and title suggestions from LLM to avoid duplicates
    const filteredLLMSuggestions = llmData.suggestions.filter(
      (s) => s.suggestion && !s.suggestion.toLowerCase().includes("title")
    );

    const allSuggestions = [
      ...deterministicSuggestions,
      ...filteredLLMSuggestions
    ];
    ...
    const otherSuggestions = allSuggestions.filter(
      (s) => s.category !== "text" && s.category !== "keyword"
    );
    ...
    // ------------------------------------------------------------------
    let currentScans = (user.scansUsed || 0) + 1;
    try {
      const { syncContact, sendTransactionalEmail } = await import('@/lib/loops');

      // Update local user stats first
      const atsScore = scoringResult.currentScore;
      ...
      // Sync user to Loops (includes atsScore, scansUsed, and new profile fields)
      if (updatedUser) {
        await syncContact(updatedUser);
      }

      // Send conditional emails
      if (currentScans === 1) {
        // First scan complete
        await sendTransactionalEmail('cmksh99ix1vj50i3ofcr6nieu', user.email, {
          name: user.name || 'Candidate',
          score: atsScore,
          dashboardLink: `${process.env.NEXT_PUBLIC_URL}/dashboard`,
        });
      } else if (currentScans === 3 && !user.isPaidUser) {
        // Third scan - upgrade nudge
        await sendTransactionalEmail('cmkshcu7d05240i222i4ql3mo', user.email, {
          name: user.name || 'Candidate',
          upgradeLink: `${process.env.NEXT_PUBLIC_URL}/pricing`,
        });
      }
    } catch (loopsError) {
      console.error('Loops integration error:', loopsError);
      // Ensure main flow doesn't break
    }
    // ------------------------------------------------------------------
...
Return the JSON now:`;
  ...
  const response = await openRouterQueue.add(() =>
    fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://shortlistai.cv",
        "X-Title": "ShortlistAI",
      },
      body: JSON.stringify({
  ...
  try {
    const result = cleanAndParseJSON(content) as LLMExtractionResult;
    return result;
  } catch (parseError) {
...
function cleanAndParseJSON(text: string): any {
  // 1. Handling Null/Undefined/Objects
  if (!text) return {};
  if (typeof text === "object") return text;

  // 2. Pre-cleaning: Remove Byte Order Mark (BOM) and zero-width spaces
  let cleaned = text.toString().replace(/[\uFEFF\u200B\u00A0]/g, "").trim();

  // 3. Remove Markdown Code Blocks (Standard Regex)
  cleaned = cleaned.replace(/^```[a-z]*\s*/i, "").replace(/```$/i, "");
  ...
  if (firstOpen !== -1 && lastClose !== -1) {
    ...
    // If no braces found, return empty object to prevent crash
    return {};
  }

  // 5. Attempt Parsing with progressive repairs
  try {
    return JSON.parse(cleaned);
  } catch (e) {
    // Stage 1 Repair: Fix Trailing Commas
    // Matches a comma followed immediately by a closing brace or bracket
    try {
      cleaned = cleaned.replace(/,(\s*[}\]])/g, "$1");
      return JSON.parse(cleaned);
    } catch (e2) {
      // Stage 2 Repair: Fix Newlines in Strings (Compatible Version)
      // The previous regex used 's' flag. This one is standard ES5 safe.
      // It finds double-quoted strings and escapes newlines inside them.
      try {
        ...
        // Stage 3 Repair: Aggressive Control Character Strip
        try {
          cleaned = cleaned.replace(/[\x00-\x1F\x7F]/g, "");
          return JSON.parse(cleaned);
        } catch (finalError) {
          console.error("JSON Critical Parse Failure. Returning empty object.");
