    ...
    try {
        ...
        if (type === "PAYMENT_SUCCESS_WEBHOOK") {
            ...
            if (transaction && transaction.status !== 'completed') {
                ...
                // ------------------------------------------------------------------
                // LOOPS.SO INTEGRATION (One-time Payment)
                // ------------------------------------------------------------------
                try {
                    ...
                    if (user) {
                        // Update user paid status and sync to Loops
                        ...
                        await sendTransactionalEmail('cmkshnmnk22vr0iya201q1mb2', user.email, {
                            ...
                            dashboardLink: `${process.env.NEXT_PUBLIC_URL}/`,
                        });
                    }
                } catch (loopsError) {
                    console.error('Loops one-time payment integration error:', loopsError);
                }
            }
        }
            ...
            try {
                ...
                await sendTransactionalEmail('cmkshnmnk22vr0iya201q1mb2', user.email, {
                    name: user.name,
                    amount: payment.payment_amount,
                    plan: getReadablePlanName(planId), // planId from subscription payload
                    credits: newTx.credits,
                    invoiceLink: `${process.env.NEXT_PUBLIC_URL}/invoices/${newTx.orderId}`,
                    dashboardLink: `${process.env.NEXT_PUBLIC_URL}/dashboard`,
                });

            } catch (loopsError) {
                console.error('Loops payment integration error:', loopsError);
            }
        }
        else if (type === "PAYMENT_FAILED_WEBHOOK" || type === "PAYMENT_USER_DROPPED_WEBHOOK") {
            const { order, payment } = body.data;
            const orderId = order.order_id;
            console.log(`[CASHFREE WEBHOOK] Payment failed/dropped for order: ${orderId}`);
            ...
            if (transaction) {
                ...
                transaction.cfPaymentId = payment?.cf_payment_id;
                await transaction.save();
                console.log(`[CASHFREE WEBHOOK] Transaction marked as failed: ${orderId}`);
            }
        }
