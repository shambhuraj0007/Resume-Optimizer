          ...
          <div className="relative">
            ...
            <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${steps.length}, 1fr)` }}>
              {steps.map((stepInfo, index) => (
                ...
                >
                  <div
                    className={`h-8 w-8 flex items-center justify-center rounded-full text-foreground font-bold mb-2 relative z-10 ${index < step
                        ? "bg-primary text-secondary"
                        : index === step
                          ? "bg-white dark:bg-background border-2 border-foreground"
                          : "bg-secondary"
                      }`}
                  >
                    {index + 1}
                  ...
                  <p
                    className={`text-xs text-center leading-tight ${index === step
            ...
            <div
              className="absolute top-[16px] left-0 right-0 h-1 -z-0"
              style={{
                marginLeft: `${100 / (steps.length * 2)}%`,
                marginRight: `${100 / (steps.length * 2)}%`
              }}
            >
              <div className="h-full w-full bg-accent rounded"></div>
              <div
