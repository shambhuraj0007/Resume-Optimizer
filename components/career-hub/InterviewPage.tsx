...
export default function InterviewPage({ post }: Props) {
    return (
        <main className="relative min-h-screen bg-gray-950 text-gray-50 overflow-hidden selection:bg-indigo-500/30">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] opacity-10 pointer-events-none">
                <div className="absolute inset-0 bg-gradient-to-b from-indigo-500 to-transparent blur-[100px] rounded-full mix-blend-screen" />
            </div>
                ...
                <header className="mb-14 mt-1">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm font-medium text-gray-300">
                        {post.primary_keyword && (
                            <span className="bg-indigo-500/10 border border-indigo-500/20 px-4 py-1.5 rounded-full shadow-sm backdrop-blur-sm text-indigo-300">
                                {post.primary_keyword}
                            </span>
                        )}
                        {post.audience && (
