...
export default function Navbar() {
  ...
  const handleSignOut = async () => {
    ...
    if (session) {
      // Use NextAuth's built-in redirect for reliable session clearing
      await signOut({ callbackUrl: "/signin?mode=signin" });
      return;
    }

    // Custom phone auth logout
    if (phoneUser) {
      await phoneLogout();
      // phoneLogout already handles redirect or we do it here
    }
        ...
        <div className="flex h-14 items-center justify-between">
          ...
          <div className="flex items-center gap-6 -ml-3">
            ...
            <div className="hidden md:flex gap-5">
              {navLinks.map((link) => {
                const href = link.href === '/pricing'
                  ? `/pricing?returnTo=${encodeURIComponent(pathname)}`
                  : link.href;
                return (
                  <Link
                    key={link.href}
