...
export async function deductCredits(
  ...
  try {
    ...
    // Check for low credits (async, don't block response)
    checkAndSendLowCreditsEmail(userId, credit.credits).catch((err: any) => console.error(err));
...
// Helper for Low Credits
async function checkAndSendLowCreditsEmail(userId: string, remainingCredits: number) {
  try {
    // TRIGGER: Only if credits are exactly 2
    // This ensures we alert them when they are running low, but don't spam on 1 or 0.
    if (remainingCredits !== 2) return;

    const user = await User.findById(userId);
    // Only send to paid users (as requested)
    if (!user || user.isPaidUser !== true) return;

    const { sendTransactionalEmail } = await import('@/lib/loops');
    // TODO: Replace 'LOW_CREDITS_TEMPLATE_ID' with actual ID
    await sendTransactionalEmail('cmktv39lg09gb0izjxw0r6ge2', user.email, {
      name: user.name,
      remaining_credits: remainingCredits,
      buy_more_link: `${process.env.NEXT_PUBLIC_URL}/pricing`
    });
    console.log(`[LOW CREDITS] Email sent to ${user.email}`);

  } catch (e) {
    console.error('[LOW CREDITS] Failed to send email:', e);
  }
}
