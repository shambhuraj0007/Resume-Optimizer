...
export async function sendContactFormEmail(name: string, email: string, subject: string, message: string) {
  // This email goes to the admin (EMAIL_USER or hello@shortlistai.cv)
  // But based on requirements, "Reply handling goes to hello@shortlistai.cv"
  // And "From" is noreply.
  // So we send TO hello@shortlistai.cv (or whatever is configured as admin email)
  // And Reply-To is the user's email.

  // Wait, the requirement says "Reply handling goes to hello@shortlistai.cv". 
  ...
  // Let's assume EMAIL_REPLY_TO (hello@shortlistai.cv) is the admin email for now, or we use a specific env var.
  // The user said: "Reply handling goes to hello@shortlistai.cv".
  // And "EMAIL_REPLY_TO=hello@shortlistai.cv".

  // For contact form:
  // To: hello@shortlistai.cv (The business)
  // From: noreply@mail.shortlistai.cv (The system)
  // Reply-To: user@example.com (The user who filled the form)

  const adminEmail = process.env.EMAIL_REPLY_TO || "hello@shortlistai.cv";
